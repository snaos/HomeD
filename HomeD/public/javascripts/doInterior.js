...
$(document).on('click', ".room-inspiration-img", function () {
    if (!roomInspirationImgs[$(this).attr("src")]) {
        ...
        $(this).css("-moz-webkit-filter", "1");
        $(this).css("-moz-filter", "grayscale(100%)");
        $(this).css("-webkit-filter", "1");
...
flowShowFunction = function () {
    ...
    $(".select-flow-show-flow-button").click(function () {
        ...
        // } else {
        //     $(".select-flow-show-flow-content").css("display", "none");
        //     $(".select-flow-show-flow-5").css("display", "block");
        //     $(".select-flow-show-flow-button").css("color", "white");
        // }
        return false;
    });
...
othersHide = function (section) {
    ...
    section = "." + section;
    if (section == ".concept-select-page" || section == ".offline-select-page") {
        $(section).css('display', '-webkit-flex');
...
inputemptyCheck = function () {
    ...
    if (document.getElementById('radio-internet').checked || document.getElementById('radio-cafe').checked || document.getElementById('radio-blog').checked || document.getElementById('radio-known').checked || document.getElementById('radio-others').checked) {
        returnValue = true;
    } else {
        returnValue = false;
    }
...
getRoomImg = function (page, callback) {
    $.ajax({
        type: "GET",
        dataType: 'json',
        url: "http://www.homed.co.kr/doInterior/select/" + page,
...
requestInterior = function () {
    ...
    if (document.getElementById('radio-internet').checked) {
        userFunnel = '인터넷검색'
    } else if (document.getElementById('radio-cafe').checked ) {
        userFunnel = '카페'
    } else if (  document.getElementById('radio-blog').checked) {
        userFunnel = '블로그'
    } else if (document.getElementById('radio-known').checked) {
        userFunnel = '지인'
    } else if (document.getElementById('radio-others').checked ) {
        userFunnel = '기타'
    }
    $.ajax({
        type: "POST",
...
clickZoomInButton = function () {
   $(document).on('click',".room-inspiration-zoom-icon",function () {
        $(".room-inspiration-img-zoom-div-img").attr('src', $(this).parent().find('.room-inspiration-img').attr('src'));
